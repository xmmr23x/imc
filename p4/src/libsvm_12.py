...
# Load the dataset
test    = pd.read_csv('../BasesDatos/csv/test_compas.csv',header=None)
X_test  = test.iloc[:,:-1].values
y_test  = test.iloc[:,-1].values

train   = pd.read_csv('../BasesDatos/csv/train_compas.csv',header=None)
...
Hp     = np.logspace(-3, 3, num=7, base=10)
optimo = model_selection.GridSearchCV(estimator=svm_model, param_grid=dict(C=Hp,gamma=Hp), n_jobs=-1,cv=10)
